...
- **`OPFSWriteAheadVFS` is only measured on Chromium and Firefox.** It opens access handles with `mode: 'readwrite-unsafe'`, recorded as unsupported for Firefox and Safari in MDN browser-compat-data (checked 2026-08-24), and unknown dictionary members are ignored rather than rejected. Without it the VFS runs in the same reduced mode as `OPFSAdaptiveVFS` — verified on Firefox 153 on 2026-08-27, where it passes every conformance invariant, concurrent writes included. **Safari has not been measured with this VFS at all**; prefer `OPFSAdaptiveVFS` there until it has been.
