...
| **`IDBBatchAtomicVFS`** | **The only persistent multi-connection VFS working on all three desktop engines.** Escapes HANDLE-1 structurally — no handle at all. Its page cache has a floor: upstream notes the cache must be large enough to hold the journal. **It serves a read during a long statement only when that statement carries a `signal` or a `timeout`** (IDB-SIGNAL, 2026-09-14, `mem:measurements`): its `jLock` holds a readwrite IndexedDB transaction that commits only when the worker yields, and since `f4b3fd7` only an abortable statement yields, on `async`/`jspi`. Without either, every other connection's read waits the statement out — which is what HANDLE-1's 2026-09-04 table recorded, on every engine. |
