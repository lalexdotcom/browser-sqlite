...
- **Global scope**: `/workspaces/wsqlite/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/wsqlite/.claude/notes/`)
...
2. Glob `/workspaces/wsqlite/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/wsqlite/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/wsqlite/.claude/notes/` — works in any directory
