...
**Result: §1.5's claim is CONFIRMED, not refuted** — on the Asyncify build as well as the
synchronous one, and on an I/O-bound query as well as a CPU-bound one. A `postMessage` is not
