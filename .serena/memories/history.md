...
| 2026-09-02 | `feat/cross-tab-coordination` | Two more lots on the same branch, the user judging them three faces of one feature. **The connection lifetime lock**: every client holds `bsq:conn` for its whole life — shared normally, exclusive where `exclusiveConnection` is declared — and `deleteDatabase` contests it, closing a measured data-loss defect where three VFS let it delete a database a client still held, silently on `IDBMirrorVFS`. **And `deleteDatabase` reports**: `DATABASE_NOT_FOUND` where it used to resolve on nothing, using `open_v2` without the create flag, uniform on all seven persistent VFS. Two new public error codes. Idempotent deletion is gone, deliberately. |
| 2026-09-01 | `feat/cross-tab-coordination` | rc.5's first item: multi-client/multi-tab, both halves. An origin-wide Web Lock per database serializes writers across every client and tab, replacing two browser-dependent behaviours with one; the commit epoch leaves the realm by being published as a lock NAME, so a commit in any tab reaches the others' next read. Both key on a storage namespace derived from `VFS_CAPABILITIES.layout`, never the VFS name. Seven tasks, subagent-driven; implementers found four defects in the plan by pushing back, and the final review found a fifth. |
| 2026-08-31 | on `main`, no branch | The type work and the surface it exposed: `exactOptionalPropertyTypes` and the seven sites it named, wa-sqlite's own types adopted with only the gaps declared here, thirty-seven `any` down to twelve. A breaking fix rode with it — `bulkWrite`'s public signature now accepts the options it documents. Plus the signal audit with its tests, the floor computed from `@mdn/browser-compat-data`, and the multi-tab Known Limitations line. Inline work, per the rule that a branch is for a feature going through the workflow. |
...
| 2026-09-02 | `feat/statement-cache-byte-bound` | The statement cache's bound moves from entries to bytes, 8 MB per worker, fed by `SQLITE_STMTSTATUS_MEMUSED` read in `settle`. The entry bound stays beside it. The rule allows one entry of overshoot and never refuses to cache, so the peak is `budget + largest statement` — and because it drops the key being re-set *before* measuring, N concurrent writers need `B > (N − 1) × MAX`, not `N × MAX`; getting that wrong would have doubled the default. Bought by measurement, not argument: a template's ceiling is ~3.4 MB and is reached by the *narrowest* table, `MEMUSED` never moves over a statement's life, and a budget too small does not degrade the cache but cancels it (+19 % Chromium, +110 % Firefox). Internal only, no option changed. Spec and plan dated 2026-09-02, two tasks by subagents. 7 commits, 555 tests. |
| 2026-09-02 | `feat/vfs-default-pragmas` | Per-VFS default PRAGMAs, declared in `VFS_CAPABILITIES.defaultPragmas`, applied by `resolvePragmas` and **generated** into the README's VFS table — a default the consumer cannot see is one they cannot refuse. Exactly one VFS clears the bar of "more performance without less reliability, sourced rather than guessed": `AccessHandlePoolVFS` takes `locking_mode=exclusive` + `journal_mode=wal`, which is free there because it is single-connection by construction, and exclusive locking is what lets SQLite use its own WAL without the `xShmMap` no VFS here implements. ~4.7x on write-transaction overhead, both engines. **Consumer pragmas are merged OVER the defaults, not substituted** — the first decision was replacement, reversed when `foreign_keys` showed what it would silently cost. Three candidates were measured and rejected: universal WAL + NORMAL, `synchronous=normal`, and `cache_size` — the last one a cap rather than a reservation, costing zero bytes and buying no measurable time. No spec: bounded, brainstormed in chat; the campaign is in `mem:measurements`. 564 tests. |
| 2026-09-03 | `feat/vfs-default-pragmas` (continued) | COOPSYNC-BUSY: a read reported busy by SQLite is retried once, which closes a defect where `OPFSCoopSyncVFS` surfaced a step of its own handle-transfer protocol as a failure — one ordinary read per session, early, **on both engines and at the default `poolSize`**, not "outside Chromium" as `mem:vfs` had inferred. Measured before designing: `OPFSAdaptiveVFS` never fails under the same workload, and the first immediate retry cleared it 7 times out of 7 in 10-17 ms, so the fix is once and without backoff. The discriminator is `sqliteCode`, not a VFS name: a `BUSY` this library mints to mean "stop" carries none and still fails fast. `stream()` and `chunk()` retry only before a row has been delivered. |
