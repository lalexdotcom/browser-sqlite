...
| **`OPFSWriteAheadVFS`** | **Its `requires: ['readwrite-unsafe']` is wrong, and the "off Chromium the pool breaks with no error" mechanism this table used to state was inferred, never executed — measured false on Firefox 2026-08-27** (all three build pairs, all six invariants, `poolSize` 1/2/4). See `mem:follow-ups` for the correction owed and why the skip hid it. Safari remains unmeasured for this VFS. |
