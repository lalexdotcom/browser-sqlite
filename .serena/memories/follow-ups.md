...
So the upstream ask is **coalesce contiguous pages into single reads and writes in `checkpoint()`** — local, no behaviour or durability change, far easier to defend than a new default. **Prototyped and measured, 2026-09-22: it reaches the floor, 0.66 -> 0.026** (CHECKPOINT-COALESCE, `mem:measurements`), with the full cell twice matching the unpatched baseline. The patch and a PR body draft are at `.scratchpad/wavfs-coalesce-2026-09-22/`; what is left is a branch on the fork and the submission, and then the report in `docs/upstream/` named with the number it gets. Three things the draft says out loud and that are not resolved: it is measured on a bulk insert, the friendliest shape for contiguous runs; the 4 MiB buffer cap is arbitrary; and no test accompanies it, because there is no behaviour to pin. Consuming the option value stays worth asking for as a separate, cheap correctness point. `page_size` is a lever we hold with no upstream at all, but it is NOT yet a recommendation: bulk insert is the friendliest case for large pages and a scattered-update workload has not been measured. The `VFS.md` paragraph saying what abort costs on this VFS is owed either way and is free.
