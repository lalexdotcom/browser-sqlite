export type SQLiteClientCallData =
	...
	| { type: 'sql'; sql: string; params?: any[]; options?: { debug?: boolean; chunkSize?: number } }
	| { type: 'abort' };

export type SQLiteCLientCallParams<K extends SQLiteClientCallData['type']> = Omit<
	Extract<SQLiteClientCallData, { type: K }>,
	'type'
>;
...
export type SQLiteWorkerMessageData<T = unknown> = {
	callId: number;
	terminate?: boolean;
} & (SQLWorkerResultData[keyof SQLWorkerResultData] | { type: 'error'; message: string });
